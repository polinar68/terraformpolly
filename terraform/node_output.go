...
func (n *nodeExpandOutput) DynamicExpand(ctx EvalContext) (*Graph, error) {
	...
	var modules []addrs.ModuleInstance
	switch {
	case n.Module.IsRoot():
		// root module don't expand
		modules = append(modules, addrs.RootModuleInstance)
	default:
		expander := ctx.InstanceExpander()
		modules = append(modules, expander.ExpandModule(n.Module)...)
	}
